...
image = (modal.Image.from_registry(
    ...
    .run_commands(["gdown 1KafnHz7ccT-3IyddBsL5yi2xGtxAKypt -O /asd/model/faceDetector/s3fd/sfd_face.pth"]))
...
def process_clip(base_dir: str, original_video_path: str, s3_key: str, start_time: float, end_time: float, clip_index: int, transcript_segments: list):
    ...
    if result.returncode != 0:
        print(f"Columbia stderr: {result.stderr}")
        raise RuntimeError(f"Columbia script failed with exit code {result.returncode}: {result.stderr[-500:]}")
